-- !!! WARNING: THIS WILL RESET ALL WORKSPACE DATA !!!
-- This script fixes the "Relation already exists" error and the "400 Bad Request" error
-- by wiping the workspace tables and starting fresh with the CORRECT schema.

-- 1. DROP EVERYTHING related to workspaces first (Clean Slate)
DROP TABLE IF EXISTS workspace_members CASCADE;
DROP TABLE IF EXISTS workspaces CASCADE;

-- 2. CREATE Tables (Correctly linked to public.profiles)
...
-- 3. Add columns to documents/spreadsheets (Safe check)
DO $$
BEGIN
    ...
    ELSE
        -- If column exists, ensuring the FK constraint exists (it might have been dropped by the CASCADE above)
        -- We'll just drop and re-add the constraint to be sure
        BEGIN
    ...
    ELSE
        BEGIN
            ALTER TABLE spreadsheets DROP CONSTRAINT IF EXISTS spreadsheets_workspace_id_fkey;
        EXCEPTION WHEN OTHERS THEN NULL; END;
        
        ALTER TABLE spreadsheets ADD CONSTRAINT spreadsheets_workspace_id_fkey 
            FOREIGN KEY (workspace_id) REFERENCES workspaces(id) ON DELETE CASCADE;
    END IF;
END $$;
...
-- 6. Apply Policies (Drop old ones first to be safe)

-- Drop ALL possible old policies for workspace stuff
...
DROP POLICY IF EXISTS "Users can view documents with workspace" ON documents;
DROP POLICY IF EXISTS "Users can view spreadsheets with workspace" ON spreadsheets;
...
-- NEW Document Policies (With Workspaces Support)
...
-- NEW Spreadsheet Policies (With Workspaces Support)
